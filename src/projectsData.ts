...
export interface Project {
    ...
    order?: number;
    /**
     * Last modification time (ms epoch) of the source markdown file,
     * read from the generated md-mtimes.json manifest. Used for the
     * "most / least recently updated" sorting of the project tiles.
     */
    updatedAt?: number;
    /**
...
export interface Experience {
    ...
    /**
     * Last modification time (ms epoch) of the source markdown file,
     * read from the generated md-mtimes.json manifest.
     */
    updatedAt?: number;
}
...
// Generated by `npm run generate:mtimes` (predev / predev:all / prebuild) — maps
// the md file path (as returned by import.meta.glob) to its last mtime in ms.
import mdTimes from './md-mtimes.json';
const summaryTimes: Record<string, number> = mdTimes.summaries;
const experienceTimes: Record<string, number> = mdTimes.experiences;
...
export const projects: Project[] = Object.entries(projectModules)
    .map(([path, module]) => {
        const project = parseProject(path, module.default);
        project.updatedAt = summaryTimes[path] ?? 0;
        return project;
    })
    .sort((a, b) => {
...
export const experiences: Experience[] = Object.entries(experienceModules)
    .map(([path, module]) => {
        const experience = parseExperience(path, module.default);
        experience.updatedAt = experienceTimes[path] ?? 0;
        return experience;
    })
    .sort((a, b) => {
